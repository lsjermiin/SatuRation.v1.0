...
string Back_translator(unsigned datatype, vector<int> seq_data) {
    unsigned number(0);
    string str("");
    
    switch (datatype) {
        case 1:
            for (vector<int>::size_type i = 0; i != seq_data.size(); ++i) {
                if (sites[i] == '1') {
                    number = seq_data[i];
                    switch (number) {
...
unsigned long Read_Input(string inname, unsigned datatype){
...
void Identify_Variant_Sites(string choice_of_sites, unsigned order, vector<int> sequence) {
    vector<int> column;
    ...
    if (toupper(choice_of_sites[0]) == 'V') {
        for (string::size_type j = 0; j != sites.size(); ++j) {
            for (string::size_type i = 0; i != taxon.size(); ++i) {
                column.push_back(alignment[i][j]);
            }
            sort(column.begin(),column.end());
            vector<int>::iterator end_unique = unique(column.begin(),column.end());
            column.erase(end_unique,column.end());
            if (column.size() == 1 || (column.size() == 2 && column[1] == order)) {
...
int main(int argc, char** argv){
    ...
    unsigned dataType(0);
    unsigned long aln_length(0), sum_dm(0), sum_diag_dm(0);
    ...
    if (toupper(choice_of_sites[0]) != 'A' && toupper(choice_of_sites[0]) != 'V') {
        cerr << "\nPROGRAM ABORTED - incorrect choice of sites: [a|v]\n" << endl;
        exit(1);
    }
    ...
    aln_length = Read_Input(inName, dataType);
    if (toupper(brevity[0]) == 'F') {
        cout << "\nProcessing data ...\n" << endl;
    }
    for (vector<vector<int> >::size_type j = 0; j != aln_length; ++j) {
        sequence.push_back(alignment[0][j]);
    }
    Identify_Variant_Sites(choice_of_sites, rows_columns, sequence);
    ...
    for (vector<vector<int> >::size_type iter1 = 0; iter1 != alignment.size(); ++iter1) {
        for (vector<vector<int> >::size_type iter2 = iter1 + 1; iter2 != alignment.size(); ++iter2) {
            ...
            for (string::size_type i = 0; i != sites.size(); ++i) {
                if (sites[i] == '1') {
    ...
    if (toupper(brevity[0]) == 'F') {
        ...
        cout << endl;
        cout << "--------------------------------------------------------------------" << endl;
        cout << "   RESULTS FROM ANALYSIS OF SATURATION COMPLETE" << endl;
        cout << endl;
        ...
        if (min_lambda == numeric_limits<double>::max() || max_lambda == -numeric_limits<double>::max()) {
            cout << "   Unexpected value of lambda - check alignment" << endl;
        } else {
            cout << "   Min(lambda) ................................ " << min_lambda << endl;
        ...
        cout << "File, # taxa, Sites, #sites, min(lambda), max(lambda), range(lambda)" << endl;
        for(string::size_type i = 0; i != sites.size(); ++i) {
            if(sites[i] == '1')
                ++varSites;;
        }
        cout << inName << "," << taxon.size() << ",";
        if (toupper(choice_of_sites[0]) == 'V') {
            cout << "Var,";
        } else {
            cout << "All,";
        ...
        cout << varSites << ",";
        if (min_lambda == numeric_limits<double>::max() || max_lambda == -numeric_limits<double>::max()) {
            cout << "Nan,Nan,Nan,ODD RESULT - check alignment" << endl;
        } else {
            cout << fixed << min_lambda << "," << fixed << max_lambda << "," << fixed << max_lambda - min_lambda << endl;
        }
    }
    return 0;
}
